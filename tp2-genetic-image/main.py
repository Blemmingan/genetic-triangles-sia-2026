...
def build_ga(target_image: Image.Image, config: dict, num_triangles: int) -> GeneticAlgorithm:
    """
    ...
    return GeneticAlgorithm(
        ...
        no_improvement_generations=config.get(
            ...
            config.get("convergence_generations", 20)
        ),
...
def main():
    ...
    # ---------------------------------------------------------
    parser.add_argument("--image-size", type=int, default=64, help="Resize target image to a square of this size")
    parser.add_argument("--output-dir", type=str, default="outputs", help="Directory where results will be saved")

    parser.add_argument("--tournament-k", type=int, help="Tournament size for deterministic tournament")
    parser.add_argument("--tournament-threshold", type=float, help="Threshold for probabilistic tournament")

    parser.add_argument("--crossover-rate", type=float, help="Probability of crossover")
    parser.add_argument("--uniform-swap-probability", type=float, help="Swap probability for uniform crossover")

    parser.add_argument("--mutation-rate", type=float, help="Mutation probability")
    parser.add_argument("--mutation-mode", type=str, choices=["reset", "delta"], help="Internal mutation mode")
    parser.add_argument("--sigma", type=float, help="Delta mutation intensity")
    parser.add_argument("--multigen-min-genes", type=int, help="Minimum scalar genes to mutate in multigen")
    parser.add_argument("--multigen-max-genes", type=int, help="Maximum scalar genes to mutate in multigen")

    parser.add_argument("--elite-fraction", type=float, help="Fraction of population used as parents")

    parser.add_argument("--fitness-threshold", type=float, help="Stop when reaching this fitness")
    parser.add_argument("--no-improvement-generations", type=int, help="Stop after N generations without improvement")
    parser.add_argument("--improvement-epsilon", type=float, help="Minimum significant improvement")

    parser.add_argument("--init-method", type=str, choices=["random", "guided"], help="Initialization method")
    parser.add_argument("--init-alpha-min", type=float, help="Minimum alpha for guided init")
    parser.add_argument("--init-alpha-max", type=float, help="Maximum alpha for guided init")
    parser.add_argument("--init-triangle-size-min", type=float, help="Minimum triangle size for guided init")
    parser.add_argument("--init-triangle-size-max", type=float, help="Maximum triangle size for guided init")
    parser.add_argument("--init-color-jitter", type=float, help="Color jitter for guided init")
    ...
    for image_path in images_to_process:
        ...
        try:
            ...
            print(f"Selection                 : {config.get('selection_method', 'tournament_deterministic')}")
            print(f"Crossover                 : {config.get('crossover_method', 'uniform')}")
            print(f"Mutation                  : {config.get('mutation_method', 'multigen')}")
            print(f"Replacement               : {config.get('replacement_method', 'additive')}")
